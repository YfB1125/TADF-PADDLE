...
x = {"key_{}".format(i): paddle.unsqueeze(paddle.to_tensor(xtrain[:, i], dtype="float32"), axis=1) for i in range(xtrain.shape[1])}
...
x = {"key_{}".format(i): paddle.unsqueeze(paddle.to_tensor(x_val[:, i], dtype="float32"), axis=1) for i in range(x_val.shape[1])}
