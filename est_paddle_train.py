...
data = []
for line in open('F://pypython_py//paddle//data//Est.dat'):
    num = float(line.strip())
    data.append(num)
smis = []
for line in open('F://pypython_py//paddle//data//smi.txt'):
    smis.append(line.strip())
vectors = []
del_mol = []
files = [int(os.path.splitext(i)[0]) for i in os.listdir('F://pypython_py//paddle//data//log//') if os.path.splitext(i)[-1] =='.log']
files.sort()
for num in files:
    mol = Chem.MolFromSmiles(smis[num - 1])
    try:
...
def train(model,X_train,Y_train,X_val,Y_val,batchsize,lr,epochs):
    ...
    optimizer = paddle.optimizer.Adam(parameters=model.parameters(),learning_rate=lr,beta1=(0.9, 0.99)[0],beta2=(0.9, 0.99)[1],weight_decay=1e-5)
    train_Loss =[]
    val_Loss = []
...
train_losses,val_losses =k_train(model,9, xtrain, ytrain, BATCH,LR,EPOCHS) #选择最优验分组
